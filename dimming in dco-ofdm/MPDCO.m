...
I_ON=2;
I_ON/sishma
I_OFF=lambda_l*sishma;
I_m=(I_ON+I_OFF)/2;
tau=M_PPM/N_sub;
Count_location=nchoosek([1:N_sub],M_PPM);
%  can we optimize N_sub and M_PPM ??
...
Gb=1;
Ptx=(sishma*lambda_l);
K=(1-qfunc(lambda_l))-qfunc(lambda_h);
...
for a=1:size(SNR,2)
    sumerr=0;
     ...
     E_n=SNR(a)-0;
     E_n_lin=10^(E_n/10);
     E_MPPM_DCO=sishma^2+ ((tau*(I_ON^2))+((1-tau)*(I_OFF^2)));
...
%          MPPM Estimation
         ...
         ext_bit_1=de2bi((find(M_location(:,1)==vec1(1) & M_location(:,2)==vec1(2)& M_location(:,3)==vec1(3)& M_location(:,4)==vec1(4)))-1,b_MPPM+1);
         ext_bit_2=de2bi((find(M_location(:,1)==vec2(1) & M_location(:,2)==vec2(2)& M_location(:,3)==vec2(3)& M_location(:,4)==vec2(4)))-1,b_MPPM+1);
         ext_bit_3=de2bi((find(M_location(:,1)==vec3(1) & M_location(:,2)==vec3(2)& M_location(:,3)==vec3(3)& M_location(:,4)==vec3(4)))-1,b_MPPM+1);
         ext_bit_4=de2bi((find(M_location(:,1)==vec4(1) & M_location(:,2)==vec4(2)& M_location(:,3)==vec4(3)& M_location(:,4)==vec4(4)))-1,b_MPPM+1);
         ext_bit_total=[ext_bit_1(1:b_MPPM)  ext_bit_2(1:b_MPPM)  ext_bit_3(1:b_MPPM)  ext_bit_4(1:b_MPPM) ];
%    DCO Estimation      
         out_fft_dco=fft(xe_dco);
     ...
     clc
     err_rate(a)=sumerr/(loop*(bc_1+bc_2));
end
figure
