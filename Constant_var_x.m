...
gamma_index=0;
for gamma=0.65:0.05:0.9
gamma_index=gamma_index+1;
PP_index=0;
PP_vect=0;
I_L=0;
I_ON=gamma*I_H; % % percent
I_OFF=(gamma-0.5)*I_H; % % percent
I_m=(I_H)/2;
%% 
lambda_l=(I_OFF-I_L)/sishma;
lambda_h=(I_m-I_OFF)/sishma;
lambda_l=(I_ON-I_m)/sishma;
lambda_h=(I_H-I_ON)/sishma;
low_vect(gamma_index)=lambda_l;
high_vect(gamma_index)=lambda_h;
...
%              ber_1
%              [vec1 vec2 vec3 vec4]
%              [loc_ON_1-1 loc_ON_2-33 loc_ON_3-66 loc_ON_4-98]
%          end
         sumerr=sumerr + ber_1 +ber_2;
     end
     cnt
     err_rate(a,gamma_index)=sumerr/(loop*(bc_1+bc_2));
end
